...
def backtest (fnopen, fnclose, vhist):
    ...
    print("[* crypto_backtest] Period Start Price: %s" % vhist['initial'])
    print()
    ...
    for vbar in vhist['history']:
        ...
        if is_open(vsymb):
            ...
            if should_open(vbar, vhist): buy(vsymb, vtrade_quantity)
...
if __name__ == "__main__":
    ...
    print("[* crypto_backtest] %s bars available." % len(vjson_hist['history']))
